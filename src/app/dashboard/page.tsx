...
function AdminDashboard({
  ...
  return (
    <div className="space-y-8">
        ...
        {STATUS_SECTIONS.map((section) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${colorMap[section.color]}`}
                >
                  <Icon size={20} />
                </div>
                <div>
      ...
      {/* Status Sections */}
      {STATUS_SECTIONS.map((section) => {
        const items = pengajuanList.filter(
          (p: any) => p.status === section.key,
        );
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${badgeMap[section.color]}`}
                >
                  {items.length}
                </span>
              </div>
...
function UserDashboard({
  ...
  const getCount = (status: string) =>
    pengajuanList.filter((s) => s.status === status).length;
  ...
  return (
    <div className="space-y-12 pb-20">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-secondary text-sm mt-2">
            Selamat datang di Smart Cemetery.
          </p>
        </div>
        <Link
        ...
        {myGraves.length > 0 ? (
              ...
              >
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <span
                      className={`px-3 py-1 text-[10px] font-bold rounded-lg uppercase tracking-widest ${
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 leading-tight mb-1">
                      {grave.deceased_name || "Lahan Dipesan"}
                    </h3>
                    <p className="text-xs text-slate-400 font-medium">
                      {grave.deceased_date
                        ? `Wafat: ${new Date(grave.deceased_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}`
                        : "Menunggu Aktivasi"}
                    </p>
                  </div>
          ...
          <div className="bg-white p-12 rounded-[2.5rem] border border-dashed border-slate-200 text-center space-y-4">
            ...
            <p className="text-secondary text-sm font-medium">
              Anda belum memiliki data lokasi makam keluarga yang terdaftar.
            </p>
            <Link
              href="/dashboard/pengajuan/baru"
              className="inline-block bg-primary text-white px-6 py-2 rounded-lg font-bold text-sm"
            >
              Daftar Sekarang
            </Link>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 pt-8">
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-slate-900 px-4">
            Statistik Saya
          </h2>
