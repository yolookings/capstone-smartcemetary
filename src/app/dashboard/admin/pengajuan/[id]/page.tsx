...
export default function PengajuanDetailPage({ params }: Props) {
  ...
  const updateStatus = async (newStatus: string, notes: string) => {
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/pengajuan?id=eq.${id}`, {
      method: "PATCH",
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
  ...
  const allocateGrave = async () => {
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/makam?pengajuan_id=eq.${id}`, {
      method: "PATCH",
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        <button
          onClick={() => router.back()}
          className="p-2.5 bg-white rounded-xl border border-slate-200 text-slate-500 hover:text-emerald-600 hover:border-emerald-300 transition-all shadow-sm cursor-pointer"
          type="button"
          aria-label="Kembali"
        >
          <ArrowLeft size={20} />
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Verifikasi dokumen dan kelola status pengajuan
          </p>
        </div>
      </div>
            ...
            {documents.length > 0 ? (
              <div className="grid md:grid-cols-2 gap-4">
                {documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="p-4 border-2 border-slate-100 rounded-xl hover:border-emerald-300 transition-all group"
                  >
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm sticky top-6">
            ...
            <div className="mb-6">
              ...
              <span
                className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold border ${getStatusColor(pengajuan.status)}`}
              >
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() =>
                  updateStatus("APPROVED", "Dokumen diverifikasi dan disetujui")
                }
